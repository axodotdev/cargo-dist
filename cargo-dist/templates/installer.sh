...
download_binary_and_run_installer() {
    ...
    # our archives have a root dir we need to step through
    # (tar has --strip-components for this but unzip has no equivalent)
    local _sub_dir = "$_dir/$_root_artifact_dir"

    for _bin_name in $_bins; do
        say "  unpacked $_bin_name"
    done

    install "$_sub_dir" "$_bins" "$@"
    local _retval=$?
