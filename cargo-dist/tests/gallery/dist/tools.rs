...
impl Tools {
    fn new() -> Self {
        ...
        let cargo_dist_path =
            std::env::var(ENV_RUNTIME_DIST_BIN).unwrap_or_else(|_| STATIC_DIST_BIN.to_owned());
        let cargo_dist =
            CommandInfo::new("cargo-dist", Some(&cargo_dist_path)).expect("dist isn't built!?");
        cargo_dist.version().expect("couldn't parse dist version!?");
