    ...
    pub(crate) fn compute_hosting(
        ...
        match hosting.hosts {
            HostingStyle::Axodotdev => {
                ...
                for ((name, version), set) in
                    releases_without_hosting.into_iter().zip(artifact_sets)
                {
                    assert_eq!(
                        name, set.package,
                        "gazenot got confused about package names..."
                    );
                    self.manifest.ensure_release(name, version).hosting = Some(Hosting::Abyss(set));
                }
            }
            HostingStyle::Github => {
                ...
                for (name, version) in releases_without_hosting {
                    ...
                    self.manifest.ensure_release(name, version).hosting =
                        Some(Hosting::Github(cargo_dist_schema::GithubHosting {
...
pub(crate) fn select_hosting(
    ...
    hosting: Option<HostingStyle>,
    ci: Option<&[CiStyle]>,
) -> Option<HostingInfo> {
    // Either use the explicit one, or default to the CI provider's native solution
    let hosting_provider = hosting.or_else(|| ci.as_ref()?.first()?.native_hosting())?;
