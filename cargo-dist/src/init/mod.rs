...
pub fn do_init(cfg: &Config, args: &InitArgs) -> DistResult<()> {
    ...
    // 1. run `dist migrate` if needed
    // 2. fetch workspace config
    // 3. initialize Cargo.toml [profile] tables if needed
    // 4. collect metadata
    // 5. load the root workspace toml file
    // 6. apply config changes in-memory
    // 7. write config to file
    // 8. apply updates to Cargo packages
    // 9. `dist generate` unless told not to
    ...
    // 7. Save the workspace toml (potentially an effective no-op if we made no edits)
    ...
    // 9. run `dist generate` unless told not to
    if !args.no_generate {
